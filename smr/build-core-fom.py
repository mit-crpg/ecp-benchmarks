...
parser.add_argument('--depleted', action='store_true',
                    help='Whether UO2 compositions should represent depleted fuel')
parser.add_argument('--no-depleted', dest='depleted', action='store_false',
                    help='Whether UO2 compositions should represent depleted fuel')
parser.add_argument('-r', '--rings', type=int, default=1,
                    help='Number of annular regions in fuel')
parser.add_argument('-a', '--axial', type=int, default=20,
                    help='Number of axial subdivisions in fuel')
parser.add_argument('-o', '--output-dir', type=Path, default=None)
parser.set_defaults(clone=True, multipole=True, depleted=True)
args = parser.parse_args()

# Make directory for inputs
if args.output_dir is None:
    if args.depleted:
        directory = Path('core-fom-depleted')
    else:
        directory = Path('core-fom-fresh')
else:
...
materials.export_to_xml(directory)


#### Create OpenMC "geometry.xml" file
geometry.export_to_xml(directory)
...
settings.export_to_xml(directory)

tallies = openmc.Tallies()
tally = openmc.Tally(name='depletion tally')
fuel_mats = [mat for mat in materials if 'UO2 Fuel' in mat.name]
tally.filters = [openmc.MaterialFilter(fuel_mats)]
tally.scores = ['total', 'absorption', 'scatter', '(n,gamma)', 'fission', '(n,2n)']
tallies.append(tally)
tallies.export_to_xml(directory)
